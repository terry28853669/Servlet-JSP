...
public class Member extends HttpServlet {
	private final String USERS = "D:/HTML5_workspace/Gossip/users";
	...
	protected void processRequest(HttpServletRequest request,
		...
		out.println("<a href='logout.do?username=" + username + "'>登出 "
				+ username + "</a>");
		out.println("</div>");
		...
		out.println("<textarea cols='60' rows='4' name='blabla'>" + blabla
				+ "</textarea>");
		out.println("<br>");
	...
	private Map<Date, String> readMessage(String username) throws IOException {
		...
		for (String txt : txts) {
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(USERS + "/" + username + "/" + txt),
					"UTF-8"));
			String text = null;
			...
			Date date = new Date(Long.parseLong(txt.substring(0,
					txt.indexOf(".txt"))));
